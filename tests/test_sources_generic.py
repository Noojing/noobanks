...
def _make_composite_adapter(data_dir=None, **kwargs) -> CompositeAdapter:
    shared = {}
    for k in ("timeout", "user_agent", "rate_limit_delay"):
        if k in kwargs:
            shared[k] = kwargs[k]
    if data_dir is not None:
        shared["data_dir"] = data_dir
    ir = IrAdapter(**shared)
    ddgs = DdgsAdapter(**shared)
    adapter_kwargs: dict = {"adapters": [ir, ddgs]}
    if data_dir is not None:
        adapter_kwargs["data_dir"] = data_dir
    for k in ("timeout", "user_agent", "rate_limit_delay"):
        if k in kwargs:
            adapter_kwargs[k] = kwargs[k]
    return CompositeAdapter(**adapter_kwargs)


def _pdf_links_html(base_url: str, *paths: str) -> str:
    """Generate HTML with <a href> links to PDFs."""
    links = "\n".join(f'<a href="{p}">{p.split("/")[-1]}</a>' for p in paths)
...
class TestExtractPdfLinks:
    ...
    def test_resolves_relative_urls(self):
        ...
        links = extract_pdf_links(html, "https://bank.example.com/ir/")
    ...
    def test_empty_html_returns_empty(self):
        links = extract_pdf_links("", "https://bank.example.com")
        assert links == []
...
class TestUrlScore:
    def test_prefers_year_in_url(self):
        score_with_year = score_candidate(
            "https://example.com/2025/report.pdf", "2025", report_type="annual_report"
        )
        score_without_year = score_candidate(
            "https://example.com/report.pdf", "2025", report_type="annual_report"
        )
    ...
    def test_prefers_annual_and_report(self):
        ...
        bad = score_candidate(
            "https://example.com/something-2025.pdf",
            "2025",
            report_type="annual_report",
        )
        assert good > bad
    ...
    def test_penalizes_cdn_urls(self):
        cdn = score_candidate(
            "https://cdn.example.com/report-2025.pdf",
            "2025",
            report_type="annual_report",
        )
        official = score_candidate(
            "https://www.example.com/report-2025.pdf",
            "2025",
            report_type="annual_report",
        )
        assert official > cdn
...
class TestReportPatterns:
    ...
    def test_nav_keywords_not_empty(self):
        ...
        assert (
            "financial-results" in NAV_KEYWORDS or "financial_results" in NAV_KEYWORDS
        )
        assert (
            "performance-report" in NAV_KEYWORDS
            or "performance_reports" in NAV_KEYWORDS
        )
...
class TestRateLimiting:
    ...
    async def test_first_call_no_delay(self):
        adapter = _make_composite_adapter(rate_limit_delay=5.0)
    ...
    async def test_second_call_is_delayed(self):
        adapter = _make_composite_adapter(rate_limit_delay=0.2)
...
class TestFetchCached:
    ...
    @pytest.mark.asyncio
    async def test_skips_download_when_exists(
        self, tmp_data_dir: Path, sample_bank_spec: BankSpec
    ):
        adapter = _make_composite_adapter(data_dir=tmp_data_dir)
        # Pre-create the target file
        target = adapter.target_path(
            tmp_data_dir, 2025, "BARC_L", "annual_report", "FY"
        )
        target.parent.mkdir(parents=True)
        target.write_bytes(b"%PDF-1.4\nfake content\n%%EOF")

        results = await adapter.fetch(sample_bank_spec, 2025, [("annual_report", "FY")])
        assert results[0].succeeded == 1
        assert results[0].report.downloaded_from == "(cached)"
    ...
    async def test_force_re_downloads(self, tmp_data_dir: Path, mocker):
        ...
        adapter = _make_composite_adapter(data_dir=tmp_data_dir)
        target = adapter.target_path(
            tmp_data_dir, 2025, "BOGUS_XX", "annual_report", "FY"
        )
        target.parent.mkdir(parents=True)
        target.write_bytes(b"%PDF-1.4\nold\n%%EOF")
        ...
        for a in adapter.adapters:
            mocker.patch.object(a, "discover_urls", return_value=[None])

        # With force=True and a bogus URL, discovery will fail → errors
        results = await adapter.fetch(bank, 2025, [("annual_report", "FY")], force=True)
...
class TestFetchNoUrls:
    ...
    async def test_returns_error_when_no_urls_discovered(
        ...
        for a in adapter.adapters:
            mocker.patch.object(a, "discover_urls", return_value=[None])
        results = await adapter.fetch(bank, 2025, [("annual_report", "FY")])
        assert results[0].succeeded == 0
        assert results[0].error is not None
...
class TestDdgsDiscoverUrl:
    ...
    @pytest.mark.asyncio
    async def test_discover_urls_returns_pdf_url(self, mocker):
        ...
        urls = await adapter.discover_urls(bank, 2025, [("annual_report", "FY")])
        assert urls[0] is not None
        assert "2025-annual-report.pdf" in urls[0]

    @pytest.mark.asyncio
    async def test_discover_urls_skips_pdf_below_score_threshold(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_discover_urls_returns_none_on_no_results(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_composite_discover_urls_calls_fallback_when_primary_returns_none(
        self, mocker
    ):
        ...
        mocker.patch.object(adapter.adapters[0], "discover_urls", return_value=[None])
        mocker.patch.object(
            adapter.adapters[1],
            "discover_urls",
    ...
    @pytest.mark.asyncio
    async def test_composite_discover_urls_skips_fallback_when_primary_succeeds(
        self, mocker
    ):
        ...
        fallback_spy = mocker.patch.object(
            adapter.adapters[1], "discover_urls", return_value=[None]
        )
...
# ── IrAdapter discover_urls ────────────────────────────────────────────────


class TestIrAdapterDiscoverUrl:
    """Tests for IrAdapter.discover_urls — unified single-URL discovery."""
    ...
    @pytest.mark.asyncio
    async def test_discover_urls_returns_none_when_crawl_empty(self, mocker):
        """When the unified crawl finds nothing, discover_urls returns None."""
        from noobanks.sources.ir_adapter import IrAdapter
        ...
        urls = await adapter.discover_urls(
            self._bank(ir_url), 2025, [("annual_report", "FY")]
        )
        assert urls[0] is None
        assert crawl_mock.call_count == 1

    @pytest.mark.asyncio
    async def test_discover_urls_returns_url_when_crawl_succeeds(self, mocker):
        """When crawl_pdf_links finds a PDF, discover_urls returns it directly."""
        from noobanks.sources.ir_adapter import IrAdapter
        ...
        assert urls[0] == crawled[0]
        assert crawl_mock.call_count == 1

    @pytest.mark.asyncio
    async def test_discover_urls_passes_both_getters_to_crawl(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_discover_urls_passes_scoring_params(self, mocker):
        """Scoring + validation pipeline is passed through to crawl_pdf_links."""
        from noobanks.sources.ir_adapter import IrAdapter
        ...
        urls = await adapter.discover_urls(
            self._bank(ir_url), 2024, [("annual_report", "FY")]
        )
        ...
        for call_args in crawl_mock.call_args_list:
            ...
            assert call_args.args[2] == [("annual_report", "FY")]
...
class TestCompositeAdapterInit:
    ...
    def test_custom_adapters_override_default_chain(self):
        ...
        class DummyAdapter(SourceAdapter):
            async def discover_urls(self, bank, year, report_specs):
                return [None] * len(report_specs)
